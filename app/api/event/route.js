...
export async function GET() {
  await ensureSchema();
  const ev = (await sql`select id, name, event_date, venue, tagline, details, flyer_url, pricing_note, pricing_deadline
  ...
  return Response.json({ id: ev.id, name: ev.name, date: ev.event_date, venue: ev.venue, tagline: ev.tagline, details: ev.details, flyer_url: ev.flyer_url, pricing_note: ev.pricing_note, pricing_deadline: ev.pricing_deadline, ticketTypes }, { headers: { 'Cache-Control': 'no-store' } });
}
