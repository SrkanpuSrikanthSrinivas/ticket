...
export async function GET(req) {
  ...
  const event = (await sql`select id, name, event_date, start_time, end_time, venue, tagline, details, flyer_url, pricing_note, pricing_deadline, food_note, email_subject, email_body
  ...
  return Response.json({ _build: 'save-fix-3', event, couponTypes, ticketTypes }, { headers: { 'Cache-Control': 'no-store' } });
}
