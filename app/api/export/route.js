...
export async function GET(req) {
  ...
  const header = 'code,name,email,mobile,country,type,qty,status,checked_in';
  const csv = [header, ...rows.map((r) =>
    [r.code, r.buyer_name, r.buyer_email, r.mobile, r.country, r.type, r.qty, r.status, r.checked_in].map(esc).join(','))].join('\n');
