...
export async function sendTicketEmail({ to, buyerName, event, ticketTypeName, code, token, qty, baseUrl }) {
  ...
  if (!process.env.SENDGRID_API_KEY) { console.warn('No SENDGRID_API_KEY set — email skipped'); return; }
  const res = await fetch('https://api.sendgrid.com/v3/mail/send', {
    method: 'POST',
    headers: { Authorization: `Bearer ${process.env.SENDGRID_API_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      personalizations: [{ to: [{ email: to }] }],
      from: { email: process.env.TICKET_FROM_EMAIL || 'tickets@mallige.org', name: event?.name || 'Tickets' },
      subject,
      content: [{ type: 'text/html', value: html }],
    }),
  });
  if (!res.ok) { const t = await res.text().catch(() => ''); console.error('SendGrid failed', res.status, t); }
}
