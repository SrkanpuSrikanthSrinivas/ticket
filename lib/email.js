...
export function buildTicketEmail({ buyerName, event, token, code, items = [], baseUrl }) {
  ...
  const totalGuests = items.reduce((s, it) => s + (it.qty || 0), 0);
  const itemRows = items.map((it) => `<tr>
      <td style="padding:8px 0;color:#1A1523;font-size:15px">${esc(it.typeName)}</td>
      <td style="padding:8px 0;text-align:right;color:#3B1E54;font-weight:700;font-size:15px">&times; ${it.qty}</td></tr>`).join('');
