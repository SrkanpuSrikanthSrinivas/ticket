...
export function buildTicketEmail({ buyerName, event, token, code, items = [], baseUrl }) {
  ...
  const fill = (t) => String(t || '')
    .replace(/\{([^}]+)\}/g, (_, k) => {
      ...
      return key in vars ? vars[key] : '';           // unknown tokens (e.g. {ticket_type}) are removed
    })
    .replace(/[ \t]{2,}/g, ' ').replace(/ +([.,;:])/g, '$1').trim();
  const subject = fill(event?.email_subject || 'Your {event} tickets');
  const bodyText = fill(event?.email_body ||
    'Your ticket for {event} is confirmed. Show the QR code below at the gate and food coupons are issued at check-in. Use your mobile number or QR code at the check-in gate.');
  ...
  const dateVenue = [event?.event_date, [event?.start_time, event?.end_time].filter(Boolean).join('–'), event?.venue].filter(Boolean).join(' · ');
...
export async function sendTicketEmail(args) {
  ...
  const provider = process.env.EMAIL_PROVIDER || (brevoKey ? 'brevo' : (resendKey ? 'resend' : (sgKey ? 'sendgrid' : 'none')));
  const name = (process.env.TICKET_FROM_NAME || '').trim() || args.event?.name || 'Tickets';

  if (provider === 'brevo' && brevoKey) {
    // Brevo (Sendinblue) transactional email API. Sender must be a verified sender
    // or on a verified domain in Brevo. Splits "Name <email>" into name + email.
    const addr = clean(process.env.TICKET_FROM_EMAIL) || 'no-reply@mallige.org';
    const m = addr.match(/^\s*(.*?)\s*<([^>]+)>\s*$/);
    const senderEmail = (m ? m[2] : addr).trim();
    const senderName = (m && m[1]) ? m[1].trim() : name;
    ...
    if (!res.ok) console.error('Brevo failed', res.status, body);
    return { ok: res.ok, status: res.status, body, provider: 'brevo', from: senderEmail };
  }
  ...
  if (provider === 'sendgrid' && sgKey) {
    ...
    const res = await fetchT(`${host}/v3/mail/send`, {
