...
export function buildTicketEmail({ buyerName, event, token, code, items = [], baseUrl }) {
  ...
  const bodyText = fill(event?.email_body ||
    'Your ticket for {event} is confirmed. Show the QR code below at the gate and food coupons are issued at check-in. Use your mobile number or the QR code at the check-in gate.');
