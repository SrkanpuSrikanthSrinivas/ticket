...
export async function ensureSchema() {
  ...
  try {
    ...
    await sql`alter table events add column if not exists convenience_fee_pct numeric not null default 0`;
    await sql`alter table orders add column if not exists fee_cents int not null default 0`;
    await sql`create table if not exists audit_log (id bigserial primary key, txn_ref text, order_id uuid, event_name text, items text, guests int not null default 0, amount_cents int not null default 0, fee_cents int not null default 0, braintree_txn_id text, status text, created_at timestamptz default now())`;
    ...
    await sql`alter table ticket_types add column if not exists min_qty int not null default 1`;
    _ensured = true;
  } catch (e) {
